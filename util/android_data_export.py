...
def android_html_data_process():
    '''export all html text as a json array'''
    result = []
    try:
        cur.execute("select Class_url, Class_html from class_html")
        lists = cur.fetchall()
...
def export_package_short_description_from_android_sdk():
    '''export the short description from jdk'''
    result = []
    try:
        cur.execute("select package_name,short_description_label from androidAPI_package")
        lists = cur.fetchall()
        for each_list in lists:
            temp = {}
            name = each_list[0]
...
def export_package_detail_description_from_android_sdk():
    result = []
    try:
